...
const DetailView = ({ selectedPostSEQ }) => {
    ...
    const closeContextMenu = (event) => {
        if (!document.getElementById("contextMenu").contains(event.target) && event.target.id !== "leftClickButton") {
    ...
    return (
        <Detail>
            <div className="board-detail" key={post?.postSEQ}>
                <div className="board-header-time">{formatDate24Hours(post?.postDate)}</div>
                <div className="board-header">
                    <div className="profile">
                        <img
                            src={
                                post?.userInfo?.profileImg ? `/uploadprofile/${post?.userInfo?.profileImg}` : defaultimg
                            }
                        />
                    </div>
                    <div className="titleNickname">
                        <div className="title">{post?.postTitle}</div>
                        <div className="nickname" id="leftClickButton" onMouseDown={handleContextMenu}>
                ...
                <div className="board-image-main">
                    <div className="board-image">
                        {attachments
                            ?.filter((attachment) => attachment.post?.postSEQ === post.postSEQ)
