...
const PostEdit = () => {
  ...
  const getPostAPI = async () => {
    ...
    // 선택한 지역 타입 정보 불러오기
    const selectedPlaceTypeData = await getSelectPlaceType(
      postData.place.placeType.placeTypeSEQ
    );
  ...
  const selectCategoryAPI = async () => {
    ...
    const selectedCategorySEQs = selectedCategoriesData.data.map(
      (item) => item.category.categorySEQ
    );
    setEditLike(selectedCategorySEQs);

    const selectedCategoryTypesData = await getCategoryTypes(
  ...
  const handleUploadImage = (e) => {
    ...
    if (selectAttach.length > 0 && temporaryData.length == 0) {
      console.log(
        "인풋으로 사진 추가 하면서 기존 삭제해야 하는 배열은 비어있을때."
      );
  ...
  const handlerDeleteImage = (e, img) => {
    const newImgUrl = imagePreviews.filter(
      (image, index) => e.target.name != index
    );
    setImagePreviews(newImgUrl);

    setSelectAttach(
      selectAttach?.filter((file, index) => e.target.name != index)
    );
    console.log(selectAttach);
    if (selectAttach[0].attachmentURL) {
      setTemporaryData([
        ...temporaryData,
        selectAttach?.filter((file, index) => e.target.name == index)[0]
          .attachmentURL,
      ]);
    } else {
      console.log(
        "attachmentURL 이 없다는 의미는 파일형식으로 받은 즉 input에서 새롭게 넣은 file형식임."
      );
    }
  };
                        ...
                        (category) => (
                          <div
                            ...
                            className={`edit-categoryLike-box-item ${
                              editLike.includes(category.categorySEQ)
                                ? "selected"
                                : ""
                            }`}
                            onClick={() =>
                              handleInterestClick(category.categorySEQ)
                            }
                          >
